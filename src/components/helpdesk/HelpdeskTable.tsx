import type { HelpdeskData } from '@/types/helpdesk';
...
interface HelpdeskTableProps {
  data: HelpdeskData[];
  onDelete: (id: string) => void;
  onEdit: (data: HelpdeskData) => void;
}
...
export function HelpdeskTable({ data, onDelete, onEdit }: HelpdeskTableProps) {
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="text-lg">Daftar Progress Helpdesk</CardTitle>
      </CardHeader>
      <CardContent>
              ...
              {data.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={9} className="text-center py-8 text-muted-foreground">
                    Belum ada data progress Helpdesk
                  </TableCell>
                </TableRow>
              ) : (
                data.map((item, index) => (
