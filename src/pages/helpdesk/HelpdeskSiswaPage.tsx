...
export function HelpdeskSiswaPage({ adminData, helpdeskData, onAddHelpdesk, onDeleteHelpdesk, onEditHelpdesk, tasks, onImportTasks, onUpdateTask }: AgentPageProps) {
  ...
  const [editingData, setEditingData] = useState<HelpdeskData | null>(null);
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);

  // State untuk peringatan data sudah ada
  const [showDuplicateWarning, setShowDuplicateWarning] = useState(false);
  const [existingData, setExistingData] = useState<HelpdeskData | null>(null);
  ...
  const handleEdit = (item: HelpdeskData) => {
    setEditingData(item);
    ...
    <div className="space-y-6">
        ...
        <CardContent className="pt-4">
          ...
          {sortedDates.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="font-medium">Belum ada Progress Helpdesk</p>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>Edit Progress Helpdesk</DialogTitle>
          </DialogHeader>
          {editingData && (
